...
describe("worktree", () => {
  ...
  beforeEach(async () => {
    ...
    const oid = repo.commit(
      repo.getTree(repo.index().writeTree()),
      "initial commit",
      {
        updateRef: "HEAD",
        author: signature,
  ...
  afterEach(async () => {
    ...
    for (const name of worktrees) {
      repo
        .findWorktree(name)
        .prune({ valid: true, locked: true, workingTree: true });
    }
  ...
  it("list worktrees", async () => {
    const worktree1 = repo.worktree(
      "worktree1",
      path.join(baseWorktreePath, "wt1-dir"),
    );
    const worktree2 = repo.worktree(
      "worktree2",
      path.join(baseWorktreePath, "wt2-dir"),
    );
  ...
  it("get worktree name and path", async () => {
    ...
    expect(await fs.realpath(worktree.path())).toBe(
  ...
  it("add worktree with checkoutExisting option", async () => {
    ...
    const worktreePath = path.join(
      baseWorktreePath,
      "test-worktree-checkout-existing",
    );
